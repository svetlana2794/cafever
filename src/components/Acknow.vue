...
<div v-else-if="storeOrder.status='ok'" class='text-2xl text-center my-4'>
Ваш заказ успешно передан.<br>

<span class='my-6 block'>Номер заказа: {{storeOrder.numOrder}}</span>

<button @click='goToMain' class='rounded-full py-2 px-10 text-2xl block mx-auto my-8 bg-[#b23a46] hover:bg-[#da1e37] text-white'>OK</button>
</div>

<div v-else-if="storeOrder.status=='error'" class='text-2xl text-center my-10'>
<p>К сожалению оформоение заказа на сайте временно не доступно. Попробуйте позже или другим способом.</p>
<Links/>
</div>
